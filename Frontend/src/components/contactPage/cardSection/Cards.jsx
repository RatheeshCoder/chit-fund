...
const Cards = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get("http://localhost:3002/branches/get");
  ...
  return (
    <>
      <div className="relative min-h-screen grid grid-cols-1 px-10 gap-10 overflow-hidden py-6 md:grid-cols-2 lg:grid-cols-3 sm:py-12">
        {branches.map((branch) => (
          <div
            key={branch._id}
            className="group relative cursor-pointer justify-around overflow-hidden bg-white px-6 pt-10 pb-8 shadow-xl ring-1 ring-gray-900/5 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl sm:mx-auto sm:max-w-sm sm:rounded-lg sm:px-10"
          >
